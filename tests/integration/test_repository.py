...
class TestBaseRepository:
    ...
    async def test_get_multi_no_filters(
        self, user_repository: UserRepository, db_session: AsyncSession
    ):
    ...
    async def test_contains_filter(
        self, populated_repository: tuple[UserRepository, list[Any]], db_session
    ):
    ...
    async def test_combined_filters(
        ...
        users, total = await user_repository.get_multi(
            filters={"is_active": True, "role": UserRole.ADMIN}
        )
    ...
    async def test_pagination_with_filters(
        ...
        page1_users, total = await user_repository.get_multi(
            skip=0, limit=1, filters={"is_active": True}
        )
        ...
        page2_users, total = await user_repository.get_multi(
            skip=1, limit=1, filters={"is_active": True}
        )
